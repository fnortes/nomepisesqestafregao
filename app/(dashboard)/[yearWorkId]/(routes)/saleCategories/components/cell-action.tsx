...
import type { SaleCategoryColumn } from "./columns";

interface Props {
  readonly data: SaleCategoryColumn;
}
...
export default function CellAction({ data }: Props) {
  ...
  const handleClickEdit = () => {
    router.push(`/${params.yearWorkId}/saleCategories/${data.id}`);
  };
  ...
  const handleConfirm = async () => {
    try {
      setLoading(true);
      await axios.delete(
        `/api/${params.yearWorkId}/saleCategories/${data?.id}`
      );
