...
import type { FoodColumn } from "./columns";

interface Props {
  readonly data: FoodColumn;
}
...
export default function CellAction({ data }: Props) {
  ...
  const handleClickEdit = () => {
    router.push(`/${params.yearWorkId}/foods/${data.id}`);
  };

  const handleConfirm = async () => {
    try {
      setLoading(true);
      await axios.delete(`/api/${params.yearWorkId}/foods/${data?.id}`);
