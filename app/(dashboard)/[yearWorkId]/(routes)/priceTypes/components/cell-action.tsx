...
import type { PriceTypeColumn } from "./columns";

interface Props {
  readonly data: PriceTypeColumn;
}
...
export default function CellAction({ data }: Props) {
  ...
  const handleClickEdit = () => {
    router.push(`/${params.yearWorkId}/priceTypes/${data.id}`);
  };

  const handleConfirm = async () => {
    try {
      setLoading(true);
      await axios.delete(`/api/${params.yearWorkId}/priceTypes/${data?.id}`);
