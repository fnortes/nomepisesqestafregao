...
import type { SaleColumn } from "./columns";

interface Props {
  readonly data: SaleColumn;
}
...
export default function CellAction({ data }: Props) {
  ...
  const handleClickEdit = () => {
    router.push(`/${params.yearWorkId}/sales/${data.id}`);
  };

  const handleConfirm = async () => {
    try {
      setLoading(true);
      await axios.delete(`/api/${params.yearWorkId}/sales/${data?.id}`);
