...
import type { SuitColumn } from "./columns";

interface Props {
  readonly data: SuitColumn;
}
...
export default function CellAction({ data }: Props) {
  ...
  const handleClickEdit = () => {
    router.push(`/${params.yearWorkId}/suits/${data.id}`);
  };

  const handleConfirm = async () => {
    try {
      setLoading(true);
      await axios.delete(`/api/${params.yearWorkId}/suits/${data?.id}`);
